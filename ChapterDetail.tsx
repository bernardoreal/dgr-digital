             ...
             <div className="transform -rotate-45 flex flex-col items-center justify-center text-white w-full h-full p-2">
                <div className="flex-1 flex items-end pb-1 mb-2">
                   <svg viewBox="0 0 24 24" fill="currentColor" className="w-14 h-14">
                     <path d="M17.56 21a1 1 0 01-.46-.11L12 18.25l-5.1 2.64a1 1 0 01-1.45-1.06l.97-5.68-4.13-4.02a1 1 0 01.55-1.7l5.7-.83 2.55-5.16a1 1 0 011.8 0l2.55 5.16 5.7.83a1 1 0 01.55 1.7l-4.13 4.02.97 5.68a1 1 0 01-.97 1.17z" />
                     <path d="M12 2C9 2 7 5 7 8c0 4 5 8 5 8s5-4 5-8c0-3-2-6-5-6z" opacity="0.8" /> 
                   </svg>
                </div>
          ...
          return (
            <div className="w-32 h-32 bg-white relative transform rotate-45 border-4 border-gray-900 shadow-xl mx-auto my-8 overflow-hidden flex items-center justify-center group hover:scale-105 transition-transform duration-300">
               ...
               <div className="absolute top-0 left-0 w-full h-1/2 overflow-hidden">
                    {[...Array(7)].map((_, i) => (
                         <div key={i} className="absolute bg-black h-full w-1" style={{left: `${(i+1)*12}%`}}></div>
                    ))}
               </div>
               <div className="transform -rotate-45 relative z-10 flex flex-col items-center justify-center w-full h-full">
            ...
            <div className="w-32 h-32 bg-white relative border-2 border-black shadow-xl mx-auto my-8 overflow-hidden flex items-center justify-center group hover:scale-105 transition-transform duration-300">
                <div className="absolute inset-0 bg-white p-2 flex flex-col justify-between">
                    ...
                    <div className="flex justify-center">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-12 h-12 text-blue-800">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
                        </svg>
                    </div>
                    <div className="text-center font-bold text-[10px] uppercase text-blue-800">Keep away from detector</div>
                </div>
            </div>
          )
      default:
  ...
  return (
    <div className={`animate-fade-in pb-32 ${isMobileView ? 'pt-4 px-2' : ''}`}>
      ...
      <div className="mb-6 flex items-start space-x-4">
        <button 
          onClick={onBack}
          className="p-2 rounded-full hover:bg-gray-200 text-gray-500 transition-colors bg-white shadow-sm border border-gray-100 mt-1"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <div className="flex items-center space-x-2 mb-1">
             <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider text-white ${chapter.color.replace('border-', 'bg-')}`}>
                 Capítulo {chapter.id}
             </span>
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">{chapter.title}</h2>
          <p className="text-gray-500 text-sm mt-1 leading-relaxed max-w-2xl">{chapter.description}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {chapter.sections.map((section) => {
            ...
            const isExpanded = activeSectionId === section.id || (initialSearchTerm && initialSearchTerm.length > 1);
            ...
            return (
              <div 
                ...
                className={`
                    ...
                    ${isExpanded ? 'border-latam-indigo ring-1 ring-latam-indigo/20' : 'border-gray-200'}
                `}
              >
                <div 
                  className={`
                    p-5 cursor-pointer transition-colors flex justify-between items-center
                ...
                >
                  <div className="flex items-center pr-4">
                    <span className={`
                        font-mono font-bold text-sm mr-3 px-2 py-1 rounded bg-white border border-gray-200 text-gray-600
                    `}>
                        {section.id}
                    </span>
                ...
                {isExpanded && (
                  <div className="px-5 py-6 animate-fade-in bg-white">
                     {section.blocks.map((block, bIdx) => (
                        <div key={`${section.id}-${bIdx}`} className="max-w-4xl">
                            {renderBlock(block, `${section.id}-${bIdx}`)}
                        </div>
                     ))}
                  </div>
                )}
      ...
      {/* Detail Modal for Blue Pages Drill-down */}
      {selectedUNEntry && (
          <UNDetailModal 
