...
const RecentQueriesPanel: React.FC<RecentQueriesPanelProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200/85 p-5 shadow-xl shadow-gray-200/20 flex flex-col h-full">
      {/* Header */}
      ...
      <div className="mt-4 flex-1 overflow-y-auto space-y-3 min-h-[220px] max-h-[450px] pr-1">
        {recentQueries.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center h-full">
            <Clock className="w-8 h-8 text-gray-300 mb-3 animate-pulse" />
          ...
          <div className="space-y-2">
            {recentQueries.map((query) => (
              <div
                ...
                className="group flex items-start p-3 hover:bg-slate-50/80 rounded-xl border border-transparent hover:border-slate-100 transition-all cursor-pointer text-left"
              >
                {/* Icon Container */}
                <div className="mt-0.5 mr-3 flex-shrink-0">
                  {query.type === 'chapter' ? (
                    <div className="bg-indigo-50 text-latam-indigo p-2 rounded-lg group-hover:bg-latam-indigo group-hover:text-white transition-all">
                      <BookOpen className="w-3.5 h-3.5" />
                    </div>
                  ) : (
                    <div className="bg-amber-50 text-amber-600 p-2 rounded-lg group-hover:bg-amber-500 group-hover:text-white transition-all">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h5 className="text-xs font-bold text-gray-800 group-hover:text-latam-indigo transition-colors truncate mt-0.5">
                    {query.title}
                  </h5>
                </div>

                {/* Action Arrow */}
                <div className="self-center ml-2 text-gray-300 group-hover:text-latam-indigo transform group-hover:translate-x-0.5 transition-all">
